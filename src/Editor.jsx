...
export function Editor({ groups, setGroups }) {
  ...
  const addOption = i => updateGroup(i,{options:[...groups[i].options,{id:uid('option'),label:'New option',color:groups[i].color}]});
  ...
  return <section className="editor panel" aria-labelledby="editor-title"><div className="section-heading"><div><span className="eyebrow">MAKE IT YOURS</span><h2 id="editor-title">Wheel editor</h2></div><button className="primary small" onClick={addGroup}>＋ Add group</button></div>
    <p className="muted">Every name, color, and option is yours to change. Your edits save automatically on this device.</p>
    <div className="group-list">{groups.map((group,gi)=><article className="group-card" key={group.id}>
      ...
      <div className="options">{group.options.map((option,oi)=><div className="option-row" key={option.id}><span className="drag">⋮⋮</span><input aria-label={`Option in ${group.label}`} value={option.label} onChange={e=>updateGroup(gi,{options:group.options.map((o,n)=>n===oi?{...o,label:e.target.value}:o)})}/><button aria-label={`Move ${option.label} up`} disabled={!oi} onClick={()=>updateGroup(gi,{options:move(group.options,oi,oi-1)})}>↑</button><button aria-label={`Move ${option.label} down`} disabled={oi===group.options.length-1} onClick={()=>updateGroup(gi,{options:move(group.options,oi,oi+1)})}>↓</button><button className="danger" aria-label={`Delete ${option.label}`} onClick={()=>updateGroup(gi,{options:group.options.filter((_,n)=>n!==oi)})}>×</button></div>)}</div>
